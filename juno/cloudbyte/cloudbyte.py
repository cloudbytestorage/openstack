...
class CloudByteISCSIDriver(SanISCSIDriver):
    ...
    def _override_default_specs(self, specs, new_specs):
        """Override the default specs with dynamic ones.
        """
        if new_specs is None:
            # Nothing to override
            return specs

        for key, value in new_specs.iteritems():
            if ':' in key:
                fields = key.split(':')
                key = fields[1]

            # will override or add a new spec
            specs[key] = value

        return specs

    def _get_properties_by_volume_type(self, context,
                                       type_id,
                                       specs=None):
        """Get the properties which can be QoS or Volume related.
        """
        ...
        # Override default specs with extra specs
        if extra_specs is not None:
            specs = self._override_default_specs(specs, extra_specs)

        # Override specs with QoS specs
        # Hence specs will prefer QoS than extra specs
        if qos_specs_id is not None:
            qspecs = qos_specs.get_qos_specs(context, qos_specs_id)['specs']
            specs = self._override_default_specs(specs, qspecs)

        # overridden specs is returned
        return specs

    def _get_group_id_from_volume(self, volumes, volume_id):
        ...
        for volume in volumes:
            if volume_id == volume['id']:
                qosgroup_id = volume['groupid']
                break

        if not qosgroup_id:
            msg = ("QoS group was not found at CloudByte "
                   "storage for volume id [%s].") % volume_id
            raise exception.VolumeBackendAPIException(msg)
    ...
    def _add_qos_group_request(self, context,
                               volume_type_id, tsmid,
                               volume_name,
                               volume_metadata):
        # Extract QoS properties from volume_metadata
        qos_props = self._get_qos_props(volume_metadata)

        # Add missing QoS props from config file
        qos_props = self._override_params(
            qos_props,
            self.configuration.add_qosgroup)

        # Fetch qos properties from volume type & override with above qos props
        if volume_type_id is not None:
            qos_props = self._get_properties_by_volume_type(
                context,
                volume_type_id,
                qos_props)

        qos_props = self._filter_qos_props(qos_props)

        # Additional requirements w.r.t QoS
        qos_props['name'] = "QoS_" + volume_name
        qos_props[self.TSM_ID] = tsmid

        # update the params with http conn info
        self._update_http_conn_params(qos_props, volume_metadata)

        data = self._api_request_for_cloudbyte("addQosGroup", qos_props)
        return data

    def _add_qos_props_to_volume_params(self, context,
                                        volume_type_id,
                                        create_volume_params,
                                        volume_metadata):
        # extract qos_props from volume_metadata
        qos_props = self._get_qos_props(volume_metadata)
        ...
        # Fetch qos properties from volume type &
        # override with above qos props
        if volume_type_id is not None:
            # get QoS properties from volume type
            qos_props = self._get_properties_by_volume_type(
                context,
                volume_type_id,
                qos_props)
        ...
        for key, value in qos_props.iteritems():
            if value is not None:
    ...
    def _add_volume_props_to_volume_params(self, context,
        ...
        # Fetch volume properties from volume type & override with above props
        if volume_type_id is not None:
            # get volume properties from volume type
            vol_props = self._get_properties_by_volume_type(
                context,
                volume_type_id,
                vol_props)
    ...
    def _create_volume_request(self, volume, volume_name,
        ...
        if self._get_ec_api_version() == 'v1':
            data = self._api_request_for_cloudbyte("createVolume", params)
        else:
            data = self._api_request_for_cloudbyte("createVolume2", params)

        return data

    def _queryAsyncJobResult_request(self, jobid, vol_metadata):
    ...
    def _filter_qos_props(self, all_props):
        qos_props = {}

        if all_props:
            qos = ['iops', 'latency', 'graceallowed', 'networkspeed',
                   'memlimit', 'tpcontrol', 'throughput', 'iopscontrol']
    ...
    def _confirm_creation_volume(self, volume_response, volume_name,
                                 vol_metadata):

        if self._get_ec_api_version() == 'v1':
            vol_res = volume_response.get('createvolumeresponse')
        else:
            vol_res = volume_response.get('createvolume2response')
        ...
        while True:
            ...
            time.sleep(self.configuration.confirm_vol_sleep_interval)
    ...
    def _build_provider_details_from_params(self, ip_address, iqn_name):
        ...
        LOG.debug("CloudByte volume [%(vol)s] properties: [%(props)s]" %
                  {'vol': iqn_name, 'props': model_update})
    ...
    def _get_ec_api_version(self):
        # Get the CB version from config
        cb_ver = (self.configuration.safe_get('cb_version') or
                  self.CB_VERSION)

        if cb_ver <= self.CB_VERSION:
            LOG.debug("Cinder will invoke version 1 of CloudByte EC API.")
            return 'v1'
        else:
            LOG.debug("Cinder will invoke version 2 of CloudByte EC API.")
            return 'v2'

    def _create_qos_group(self, context, volume_type_id,
                          create_volume_params, volume_name,
                          vol_metadata):
        ...
        if self._get_ec_api_version() == 'v1':
            ...
            LOG.debug("Successfully created qos group for CloudByte "
                      "volume [%s]", volume_name)
        else:
            self._add_qos_props_to_volume_params(
                context,
                volume_type_id,
                create_volume_params,
                vol_metadata)
    ...
    def create_volume(self, volume):
        ...
        success_job_response = (
            self._confirm_creation_volume(vol_data, volume_name, vol_metadata)
            )
    ...
    def update_volume_metadata(self, volume, metadata):
        ...
        if cb_volume_id != 'None':
            ...
            cb_volumes = cb_volumes['listFilesystemResponse']
            cb_volumes = cb_volumes.get('filesystem')

            # extract the qosgroup id from above list
            group_id = self.self._get_group_id_from_volume(
                cb_volumes,
                cb_volume_id)

            # prepare params for qos update API
            qos_params = self._filter_qos_props(vol_metadata)
    ...
    def retype(self, admin_context, volume, new_type, diff, host):
        """Retypes a volume, QoS update is only done for the time being."""
        ...
        # during volume creation.
        vol_metadata = self.db.volume_metadata_get(
            admin_context, source_volume_id)

        # Find CB volume id w.r.t. openstack source volume
        cb_volume_id = vol_metadata['cb_volume_id']

        if cb_volume_id is None:
            raise exception.VolumeBackendAPIException(
                "CloudByte volume details (i.e. metadata) is not "
                "available for its corresponding OpenStack "
                "volume [%s]", source_volume_id)

        params = self._get_properties_by_volume_type(
            admin_context,
            new_type['id'],
            None)

        qos_params = self._filter_qos_props(params)

        if qos_params is not None and len(qos_params) > 0:
            # Get volume details
            ...
            # Extract qos group id w.r.t CloudByte volume
            qosgroup_id = self._get_group_id_from_volume(
                cb_volumes,
                cb_volume_id)
            qos_params['id'] = qosgroup_id

            # update the params with http conn info
            self._update_http_conn_params(qos_params, vol_metadata)
            self._api_request_for_cloudbyte('updateQosGroup', qos_params)

            LOG.info("Successfully retyped CloudByte volume [%(cb_vol)s] "
                     "w.r.t OpenStack volume [%(stack_vol)s].",
                     {'cb_vol': cb_volume_id,
                      'stack_vol': source_volume_id})
        return True
